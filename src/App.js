...
function App() {
	...
	useEffect(() => {
		fetch('https://washyka-server.herokuapp.com/allOrders').then((res) => res.json()).then((data) => setOrder(data));
	}, [order.length]);

	useEffect(() => {
		fetch('https://washyka-server.herokuapp.com/products').then((res) => res.json()).then((data) => setProducts(data));
	}, [products.length]);
